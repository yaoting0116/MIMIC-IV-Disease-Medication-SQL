...
def show():
    ...
    query_names_subtitle = [
        # SQL Step One.
        "Use NATURAL JOIN to combine hosp (diagnoses_icd, admissions). # Produces the complete hosp table (IDs not deduplicated).",
        # SQL Step Two.
        ...
        # SQL Step Four.
        "Use subject_id from temp_two (hosp) as a query condition for temp_three (ed). # Find in ed those with the same subject_id as in hosp.",
        # SQL Step Five.
        "Use UNION ALL to merge temp_one (hosp) + temp_four (ed). # Produces the complete combined hosp + ed table (IDs not deduplicated).",
        # SQL Step Six.
        ...
        """1.Use GROUP BY subject_id to group records, ensuring no duplicate subject_id.
            ...
            \n3.Use DELETE FROM to remove records where psychosis is TRUE and the earliest admission date equals the last admission date.""",
        # SQL Step Ten.
        ...
        """1.Create table temp_eleven by importing data from temp_nine using CREATE TABLE.
            \n2.Use DELETE FROM to remove records from temp_nine (entire) where the earliest admission date equals the ischemic stroke earliest admission date.""",
        # SQL Step Twelve.
        ...
        """1.Create table temp_twenty_two by reading from temp_twenty_one and performing a new SELECT query.
            \n2.Use the CASE statement to determine whether the patient has a specific disease and set the value to TRUE or FALSE based on the result.
            \n3.Use the COALESCE function to replace NULL values in disease counts with 0, preventing errors in subsequent calculations.
            ...
            \n7.Use LEFT JOIN to merge the records of each patient, ensuring that even if there is no matching disease record, the patient’s data is retained.""",
...
SELECT * FROM temp_twenty_three;"""
    ...
    st.title("📊 Disease-Specific SQL Test Queries")
    if data_dict:
